...
class _QuizAreaState extends State<QuizArea> {
  void newQuiz() async {
    ...
    Map<String, dynamic> currentKanji = currentStage[stageItem];
    ...
    // selecting question and answer type
    if (stage == 0 && stageAttempts[0][stageItem] == 0) {
      // answer type is based on flowchart
      // if (on present) {questionType = 0; answerType = 0;} else if (semantic) {questionType = 1; answerType = 2} else {show onyomi + meaning}
    } else {
      questionType = random.nextInt(2);
      answerType = random.nextInt(2);
    }

    // setting values for question types
    String kanji = currentKanji['literal'];
    List kanjiKnownRads = currentKanji['user_radicals']; // radicals the user knows in the kanji, can be null
    String kanjiVgId = kanjiToId[kanji]!;
    if (questionType == 0) {
      List answerList = [];
      String questionText = 'What is the ${answerTypes[answerType]} reading?'; // question type
      String answerText = currentKanji['meanings'].join(', ');
      if (answerType == 0 ) {answerList = currentKanji['onreadings'];}
      if (answerType == 1 ) {answerList = currentKanji['kunreadings'];}
      if (answerType == 2 ) {
        answerList = currentKanji['meanings'];
        questionText = currentKanji['meanings'].join(', ');
        answerText = 'onyomi: ${currentKanji['onreadings'].join(', ')} | kunyomi: ${currentKanji['onreadings'].join(', ')}';
        }
      int ansIndex = random.nextInt(answerList.length);
      String answer = answerList[ansIndex];
      String altAnswers = '';
      answerList.remove(answer);
      altAnswers = answerList.join(', ');
  ...
  Future<void> initializeKanji() async {
    ...
    var accuracy = onData[0]['on_accuracy'];
    if (accuracy == Null) {print('null');accuracy = 0;}

    try {
       if (accuracy >= 70 && onData[0]['learned kanji'] >= 3) {
      try {
        ...
        if (e is TypeError) { // if kunyomi is null
